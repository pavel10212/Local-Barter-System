...
const MyItems = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const body = selectedItem ? { ...newItem, id: selectedItem.itemId } : newItem;
            const response = await fetch(url, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(body),
            });
            if (!response.ok) throw new Error(`Failed to ${selectedItem ? 'edit' : 'create'} item`);
            const updatedItem = await response.json();
            setItems(prev => selectedItem
                ? prev.map(item => item.itemId === selectedItem.itemId ? updatedItem : item)
                : [...prev, updatedItem]
            );
            handleDialogClose();
        } catch (error) {
        ...
        <div className="bg-gradient-to-br from-gray-900 to-gray-800 min-h-screen text-white">
            ...
            <main className="container mx-auto px-4 py-8">
                <div className="flex justify-between items-center mb-6">
                    ...
                    <Button onClick={() => handleDialogOpen()} className="bg-black text-white hover:bg-white hover:text-black">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {items.map((item) => (
                        <Card key={item.id} className="bg-gray-700 text-white cursor-pointer hover:shadow-xl transition duration-300 transform hover:-translate-y-1" onClick={() => handleDialogOpen(item)}>
                            <CardHeader>
                            ...
                            <CardContent>
                                <div className="h-48 bg-gray-600 flex items-center justify-center mb-4">
                                    {item.image ? (
                                        <Image width={400} height={400} src={item.image} alt={item.name} className="h-full w-full object-cover" />
                                    ) : (
            ...
            <Dialog open={isDialogOpen} onOpenChange={handleDialogClose}>
                <DialogContent className="bg-gray-800 text-white">
                    ...
                    <DialogDescription className="text-center text-gray-400 mb-6">
                        {selectedItem ? 'Update the details of your item.' : 'Enter the details of your new item.'}
                    </DialogDescription>
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div>
                            <label htmlFor="name" className="block text-sm font-medium mb-2">Item Name</label>
                            <Input
                                id="name"
                                name="name"
                                value={newItem.name}
                        ...
                        <div>
                            <label htmlFor="image" className="block text-sm font-medium mb-2">Item Image</label>
                            <Input
                                id="image"
                                name="image"
                                type="file"
                            ...
                            {previewImage && (
                                <div className="mt-4">
                                    <Image width={400} height={400} src={previewImage} alt="Preview" className="w-full h-48 object-cover rounded" />
                                </div>
                            )}
                        </div>
